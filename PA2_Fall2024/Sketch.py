    ...
    def Interrupt_Keyboard(self, keycode):
        if keycode in [wx.WXK_RETURN]:
            ...
            if len(self.components) > 0:
                ...
                if self.select_obj_index >= 0 and self.select_obj_index not in self.selected_components:
                ...
                # Set new selected component's color (if not already selected)
            ...
            # Switch to the last rotation axis for all selected components
            self.select_axis_index = (self.select_axis_index - 1) % 3
            for index in self.selected_components:
                self.components[index].setCurrentColor(self.select_color[self.select_axis_index])
            self.update()

        elif keycode in [wx.WXK_RIGHT]:
            # Switch to the next rotation axis for all selected components
            self.select_axis_index = (self.select_axis_index + 1) % 3
            for index in self.selected_components:
                self.components[index].setCurrentColor(self.select_color[self.select_axis_index])
            self.update()
            ...
            # Rotate all selected components clockwise
            if len(self.selected_components) > 0:
                for index in self.selected_components:
                    if index in self.alist[self.select_axis_index]:
                        self.components[index].rotate(self.MOUSE_SCROLL_SPEED, self.components[index].axisBucket[self.select_axis_index])
                    else:
                        self.components[index].rotate(-self.MOUSE_SCROLL_SPEED, self.components[index].axisBucket[self.select_axis_index])
            self.update()

        elif keycode in [wx.WXK_DOWN]:
            # Rotate all selected components counterclockwise
            if len(self.selected_components) > 0:
                for index in self.selected_components:
                    if index in self.alist[self.select_axis_index]:
                        self.components[index].rotate(-self.MOUSE_SCROLL_SPEED, self.components[index].axisBucket[self.select_axis_index])
            ...
            # Exit component editing mode and reset all selected components
            for index in self.selected_components:
            ...
            # Loop over all components (assuming these are joints)
            for index, component in enumerate(self.components):
                # Assuming each component has a method or attributes for getting rotation angles:
                # For example, component.getRotation() -> [u_angle, v_angle, w_angle]
                # You need to replace this with the actual way you access the component's angles
                u_angle = component.getCurrentAngle(component.uAxis)
                v_angle = component.getCurrentAngle(component.vAxis)
                w_angle = component.getCurrentAngle(component.wAxis)

                # Create a list with the index and the 3 angles of rotation
                joint_data = [index, u_angle, v_angle, w_angle]

                # Add this joint data to the main list
                joint_angle_list.append(joint_data)

            # Print the final list of joint rotations
            print(joint_angle_list)

        elif chr(keycode) in "R":
            # Reset everything, including the selection
        ...
        elif chr(keycode) in "m":
            # Select multiple limbs/components
            if self.select_obj_index >= 0:
                if self.select_obj_index in self.selected_components:
                    # Deselect component
                    self.selected_components.remove(self.select_obj_index)
                    self.components[self.select_obj_index].reset("color")
                else:
                    # Select component
                    self.selected_components.append(self.select_obj_index)
                    self.components[self.select_obj_index].setCurrentColor(self.select_color[self.select_axis_index])
            self.update()
